...
cp <PATH TO SLE MICRO SELF INSTALL ISO> eib_quickstart/base-images/
...
docker run --privileged --rm -it -v $PWD/eib_quickstart/:/eib registry.opensuse.org/home/atgracey/eib/container/containerfile/eib:latest /usr/bin/eib -config-file eib-config.yaml -config-dir /eib -build-dir /eib/_build
...
If you are booting a physical device, we need to burn the image to a USB flash drive. This can be done by:
